...
async def list_stages(db: AsyncSession = Depends(get_db)):
    ...
    if not stages:
        ...
        fallback_titles = {
            1: ("Документы и доступы", "Документы", "Подписание документов, получение доступов, настройка MPulse и изучение базы знаний Confluence.", "Ачивка «Старт»", "Откроется после прохождения этапа", "docs"),
