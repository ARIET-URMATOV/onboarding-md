...
export function RegisterPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setError(null);
    if (!name || !email || !password) { setError('Р—Р°РїРѕР»РЅРё РІСЃРµ РїРѕР»СЏ'); return; }
    if (password.length < 6) { setError('РџР°СЂРѕР»СЊ РґРѕР»Р¶РµРЅ Р±С‹С‚СЊ РЅРµ РєРѕСЂРѕС‡Рµ 6 СЃРёРјРІРѕР»РѕРІ'); return; }
    setLoading(true);
    ...
    <div className="auth-wrap">
      <div className="auth-card glass-strong">
        <div className="auth-head">
          ...
          <h1 className="font-orbitron">РЎРћР—Р”РђРўР¬ РђРљРљРђРЈРќРў</h1>
          <p>РќР°С‡РЅРё СЃРІРѕР№ РїСѓС‚СЊ РІ РєРѕРјР°РЅРґРµ MDIGITAL</p>
        </div>

        <form onSubmit={onSubmit}>
          <label className="field">
            <span>РРјСЏ</span>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="РђР»РµРєСЃРµР№ РРІР°РЅРѕРІ" required />
          </label>
          <label className="field">
            <span>Email</span>
          ...
          <label className="field">
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="РњРёРЅРёРјСѓРј 6 СЃРёРјРІРѕР»РѕРІ" required minLength={6} />
          </label>
          {error && <div className="error">{error}</div>}
          <button type="submit" className="btn-primary" disabled={loading}>
            {loading ? 'РЎРѕР·РґР°С‘РјвЂ¦' : 'РЎРћР—Р”РђРўР¬ РђРљРљРђРЈРќРў в†’'}
          </button>
        </form>
